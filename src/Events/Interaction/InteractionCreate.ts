...
import { Event, Client } from '@/Structures';
...
export default new Event(Events.InteractionCreate, async (client, interaction) => {
    ...
    if (interaction.isChatInputCommand()) {
        ...
        if (subCommandGroup) {
            ...
            try {
                Logger.info(`${chalk.bold(interaction.user.tag)} has used ${chalk.yellow(interaction.commandName)} on ${interaction.guild ? `${interaction.guild.name} (${interaction.guild.id})` : 'Direct Message (DM)'}.`);
        ...
        if (componentPermissionHandler && !componentPermissionHandler.includes(interaction.user.id)) {
            try {
                interaction.reply({
                    embeds: [
                        new EmbedBuilder()
                            .setDescription('That button is not for you.')
                            .setColor('Red')
                    ],
                    ephemeral: true
                });
        ...
        try {
            ...
            err instanceof Error ? Logger.trace(err.stack) : Logger.error(err);
        }
        ...
        if (componentPermissionHandler && !componentPermissionHandler.includes(interaction.user.id)) {
            try {
                interaction.reply({
                    embeds: [
                        new EmbedBuilder()
                            .setDescription('That menu is not for you.')
                            .setColor('Red')
                    ],
                    ephemeral: true
                });
